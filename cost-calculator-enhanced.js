...
(function () {
  ...
  ready(function () {
    ...
    var priceData = (window.SHELLZ_DATA && window.SHELLZ_DATA.pricing && window.SHELLZ_DATA.pricing.plans) || {};
    function price(key, field) {
      var value = priceData[key] && priceData[key][field];
      return value === null || value === undefined ? '' : String(value);
    }
    var presets = {
      under25: { planLabel: 'Manual under-$25 plan', websiteType: 'learning', hostingType: 'shared', budgetLimit: '25', billingMonths: '12', domainNeeded: 'yes', emailNeeded: 'no', introPrice: '', renewalPrice: '', domainCost: '12.00', emailCost: '0.00', addonCost: '0.00' },
      blog: { planLabel: 'Manual beginner blog plan', websiteType: 'blog', hostingType: 'wordpress', budgetLimit: '50', billingMonths: '12', domainNeeded: 'yes', emailNeeded: 'no', introPrice: '', renewalPrice: '', domainCost: '12.00', emailCost: '0.00', addonCost: '0.00' },
      business: { planLabel: 'Bluehost Starter research example', websiteType: 'business', hostingType: 'wordpress', budgetLimit: '100', billingMonths: priceData['bluehost-starter'] && priceData['bluehost-starter'].introTermMonths ? String(priceData['bluehost-starter'].introTermMonths) : '12', domainNeeded: 'yes', emailNeeded: 'basic', introPrice: price('bluehost-starter','introMonthly'), renewalPrice: price('bluehost-starter','renewalMonthly'), domainCost: '0.00', emailCost: '12.00', addonCost: '0.00' },
      affiliate: { planLabel: 'Manual affiliate-site plan', websiteType: 'affiliate', hostingType: 'wordpress', budgetLimit: '250', billingMonths: '12', domainNeeded: 'yes', emailNeeded: 'no', introPrice: '', renewalPrice: '', domainCost: '12.00', emailCost: '0.00', addonCost: '0.00' }
    };
    ...
    function calculate() {
      ...
      var hasIntroInput = String(el('introPrice') && el('introPrice').value || '').trim() !== '';
      var hasRenewalInput = String(el('renewalPrice') && el('renewalPrice').value || '').trim() !== '';
      ...
      if (!hasIntroInput || !hasRenewalInput) {
        set('firstYearCost', '—');
        set('renewalYearCost', '—');
        set('twoYearCost', '—');
        set('renewalIncrease', '—');
        if (summary) summary.textContent = 'Enter exact-plan intro and renewal prices to calculate';
        if (warning) {
          warning.textContent = 'Data needed: enter both the intro monthly equivalent and the renewal monthly equivalent for the same exact plan before relying on this estimate.';
          setClass(warning, 'warn');
        }
        if (recommendation) recommendation.textContent = 'Copy the intro price, renewal price, and billing term from the same exact plan. Do not mix provider-wide prices or different plan tiers.';
        if (avoid) avoid.innerHTML = ['Use the same exact plan for intro and renewal values.','Match the prepaid term before comparing totals.','Add domain, email, backups, and paid extras separately.'].map(function (item) { return '<span>' + item + '</span>'; }).join('');
        return;
      }
      ...
      if (summary) {
        if (firstTerm <= budget) summary.textContent = 'This setup fits your selected first-term budget';
        else summary.textContent = 'This setup is above your selected first-term budget';
      }
